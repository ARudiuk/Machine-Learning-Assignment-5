...
errors = np.zeros((11,6))
for i in range(-5,6):    
    print i
    learner = SVM(kernel='linear',C=2**(i))
    learner.train_svm(split_TRAIN_DATA,np.reshape(TRAIN_LABEL,(np.shape(TRAIN_LABEL)[0],)))
    errors[i+5,0] = learner.test_svm(split_TRAIN_DATA,np.reshape(TRAIN_LABEL,(np.shape(TRAIN_LABEL)[0],)))
    learner = SVM(kernel='rbf',C=2**(i))
    learner.train_svm(split_TRAIN_DATA,np.reshape(TRAIN_LABEL,(np.shape(TRAIN_LABEL)[0],)))
    errors[i+5,1] = learner.test_svm(split_TRAIN_DATA,np.reshape(TRAIN_LABEL,(np.shape(TRAIN_LABEL)[0],)))
    learner = SVM(kernel='sigmoid',C=2**(i))
    learner.train_svm(split_TRAIN_DATA,np.reshape(TRAIN_LABEL,(np.shape(TRAIN_LABEL)[0],)))
    errors[i+5,2] = learner.test_svm(split_TRAIN_DATA,np.reshape(TRAIN_LABEL,(np.shape(TRAIN_LABEL)[0],)))   
    learner = SVM(kernel='poly',degree=2,C=2**(i))
    learner.train_svm(split_TRAIN_DATA,np.reshape(TRAIN_LABEL,(np.shape(TRAIN_LABEL)[0],)))
    errors[i+5,3] = learner.test_svm(split_TRAIN_DATA,np.reshape(TRAIN_LABEL,(np.shape(TRAIN_LABEL)[0],)))   
    learner = SVM(kernel='poly',degree=3,C=2**(i))
    learner.train_svm(split_TRAIN_DATA,np.reshape(TRAIN_LABEL,(np.shape(TRAIN_LABEL)[0],)))
    errors[i+5,4] = learner.test_svm(split_TRAIN_DATA,np.reshape(TRAIN_LABEL,(np.shape(TRAIN_LABEL)[0],)))       
    learner = SVM(kernel='poly',degree=4,C=2**(i))
    learner.train_svm(split_TRAIN_DATA,np.reshape(TRAIN_LABEL,(np.shape(TRAIN_LABEL)[0],)))
    errors[i+5,5] = learner.test_svm(split_TRAIN_DATA,np.reshape(TRAIN_LABEL,(np.shape(TRAIN_LABEL)[0],)))    

np.save("svm_results_train",errors)
